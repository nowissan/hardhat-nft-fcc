...
!developmentChains.includes(network.name)
    ...
    : describe("Dynamic SVG NFT", function () {
          let dynamicSvgNft, deployer, mockV3Aggregator, highValue, lowValue
          beforeEach(async function () {
              ...
              highValue = ethers.utils.parseEther("0.1")
              lowValue = ethers.utils.parseEther("100000")
          })

          describe("constructor", function () {
              it("initializes ERC721, imageUri and pricefeeds successfully", async function () {
          ...
          describe("mintNft", function () {
              it("can mint NFT for high value", async function () {
                  await expect(dynamicSvgNft.mintNft(highValue)).to.emit(
                      dynamicSvgNft,
                      "CreatedNFT"
                  )
                  const tokenIdHighValue = await dynamicSvgNft.getHighValue(0)
                  const tokenCounter = await dynamicSvgNft.getTokenCounter()
                  assert.equal(
                      tokenIdHighValue.toString(),
                      highValue.toString()
                  )
                  ...
                  await expect(dynamicSvgNft.tokenURI(99)).to.be.revertedWith(
                      "URI Query for nonexistent token"
                  )
              })
              it("can mint NFT for low value", async function () {
